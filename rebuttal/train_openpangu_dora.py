...
def compute_auc_intervals(losses):
    intervals = {'auc_0_100': (0, 100), 'auc_100_500': (100, 500), 'auc_0_500': (0, 500), 'auc_0_2500': (0, 2500)}
...
def get_args():
    ...
    ap.add_argument("--device", type=str, default="npu:2")
...
def main():
    ...
    peak_memory, training_log, training_losses, test_losses_history, best_loss = start_memory, [], [], [], float('inf')
    ...
    for step in range(1, args.max_iters + 1):
        ...
        current_mem = torch_npu.npu.memory_allocated(device) if device_type == 'npu' else (torch.cuda.memory_allocated(device) if torch.cuda.is_available() else 0)
        peak_memory = max(peak_memory, current_mem)
        
        log_entry = {'step': step, 'train_loss': current_loss, 'lr': scheduler.get_last_lr()[0], 'grad_norm': grad_norm, 'mem_gb': (current_mem - start_memory)/1e9, 'test_loss': None}
