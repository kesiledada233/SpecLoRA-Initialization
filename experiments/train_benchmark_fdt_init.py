"""
OpenPangu FDT 初始化验证脚本 - 支持 4 数据集 + 效率监控
...
运行示例:
  ...
  python train_benchmark_fdt_init.py \
      ...
      --lora_r 16 \
      --out_dir outputs_gsm8k_ablation_baseline_r16 \
      --device npu:1
...
DATASET_PATHS = {
    ...
    'mbpp': '/root/nvme0n1/Noneq_Neural_Network/pretrained_models/mbpp/processed',  # 新增
}
...
def main():
    ...
    print(f"[FDT] ✓ 初始化信息: {init_info_file}\n")
    ...
    try:
        ...
        if len(test_dataset_raw) > max_test_samples:
            ...
            print(f"[数据] 限制测试集: {max_test_samples} 样本")
    ...
    for step in range(1, args.max_iters + 1):
        ...
        if step % args.eval_interval == 0 or step == 1:
            ...
            log += f", lr={lr_current:.2e}, {elapsed:.1f}s"
            log += f", Mem={memory_gb:.2f}GB{auc_str}"
    ...
    if len(training_losses) >= 500:
        print(f"AUC(0-500): {sum(training_losses[:500]):.2f}")
