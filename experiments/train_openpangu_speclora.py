...
def get_args():
    ...
    ap.add_argument("--fda_method", type=str, default='fft', choices=['fft', 'ar'])
    ap.add_argument("--unroll_order", type=str, default='row', choices=['row', 'col'], help="矩阵展开顺序")
    ap.add_argument("--verify_fda", action="store_true")
    ...
    # 参数逻辑校验：如果传了兼容的 --use_fda_init，强制转换为 --init_method fda
    if args.use_fda_init:
        args.init_method = 'fda'
...
def main():
    ...
    # 解析 FDA 预设 (如果有)
    if args.init_preset and args.init_method in ['peft_default', 'fda']:
        preset_configs = {
            'baseline': {'init_method': 'peft_default', 'alpha': None, 'name': 'PEFT Default'},
            'soft': {'init_method': 'fda', 'alpha': 0.8, 'name': 'FDA-Soft (α=0.8)'},
            'medium': {'init_method': 'fda', 'alpha': 1.1, 'name': 'FDA-Medium (α=1.1)'},
            'strong': {'init_method': 'fda', 'alpha': 1.5, 'name': 'FDA-Strong (α=1.5)'},
        }
        config = preset_configs[args.init_preset]
        print(f"[预设] 触发预设配置: {config['name']}")
        args.init_method = config['init_method']
        if args.init_method == 'fda':
            args.fda_alpha = config['alpha']
    ...
    if args.init_method == 'pissa':
        ...
        # peft_default 和 fda 在 LoraConfig 阶段都使用默认初始化
        pass
